...
class MainViewModel (val weatherRepository: WeatherRepository,application: Application )
    ...
    val currentWeatherResult:MutableLiveData<BaseResponse<CurrentWeatherResponse>> =
            MutableLiveData()
    val currentWeatherLocationResult:MutableLiveData<BaseResponse<CurrentWeatherResponse>> =
        MutableLiveData()
    ...
    fun getCurrentWeatherByLocation(latitude:String,longitude:String){
        viewModelScope.launch {
            currentWeatherLocationResult.postValue(BaseResponse.Loading())
            val response = weatherRepository.getCurrentWeatherByLocation(latitude, longitude)
